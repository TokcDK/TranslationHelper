        ...
        internal static async void OpenProject(string filePath = null)
        {
            if (filePath == null || !File.Exists(filePath))
            {
                using (var THFOpen = new OpenFileDialog())
                {
                    ...
                    var possibleExtensions = string.Join(";*", GetListOfSubClasses.Inherited.GetInterfaceImplimentations<IFormat>().Where(f => !string.IsNullOrWhiteSpace(f.Extension) && f.Extension[0] == '.').Select(f => f.Extension).Distinct());
