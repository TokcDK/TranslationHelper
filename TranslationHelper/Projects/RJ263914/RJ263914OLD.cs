...
namespace TranslationHelper.Projects.RJ263914
{
    ...
    class RJ263914OLD
    {
        ...
        {
            AppData.CurrentProject.SelectedGameDir = GameDirectory;
        ...
        private void RubyRPGGameFIlesFromTheDir(string targetDirPath, string extension, bool IsWrite = false)
        {
            ...
            foreach (var filePath in Directory.EnumerateFiles(targetDirPath, "*." + extension, SearchOption.AllDirectories))
            {
                ...
                bool tableNotExists = AppData.CurrentProject.FilesContent.Tables[tableName] == null;
                if (tableNotExists)
                {
                    if (IsWrite)
                    {
                ...
                else if (folderName == "pants")
                {
                    ...
                    {
                        AppData.CurrentProject.FilesContent.Tables[tableName].Rows.Add(lineValue);
                        AppData.CurrentProject.FilesContentInfo.Tables[tableName].Rows.Add(fileName);
                    }
                    processingFolderName = folderName;
                    rowIndex++;
                }
                ...
                else if (folderName == "type")
                {
                    ...
                    if (lines.Length == 2 && !string.IsNullOrEmpty(lines[1]) && !lines[1].IsDigitsOnly())
                    {
                        ...
                        {
                            AppData.CurrentProject.FilesContent.Tables[tableName].Rows.Add(lines[1], lines[0]);
                            AppData.CurrentProject.FilesContentInfo.Tables[tableName].Rows.Add(fileName);
                        }
                        processingFolderName = folderName;
                        rowIndex++;
                    ...
                    if (line.Length > 0)
                    {
                        ...
                        if (lineArray.Length > 4)
                        {
                            ...
                            if (!string.IsNullOrEmpty(lineArray[4]) && !lineArray[4].IsDigitsOnly())
                            {
                                if (IsWrite)
                                {
                                    var row = AppData.CurrentProject.FilesContent.Tables[fileName].Rows[rowIndex];
                                ...
                                {
                                    AppData.CurrentProject.FilesContent.Tables[fileName].Rows.Add(lineArray[4]);
                                    AppData.CurrentProject.FilesContentInfo.Tables[fileName].Rows.Add(folderName + Path.DirectorySeparatorChar + fileName);
                                }
                            }
                        }
                    }
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (line.Length > 0 && line.StartsWith("message"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(lines[1]) && !lines[1].IsDigitsOnly())
                        {
                            ...
                            {
                                AppData.CurrentProject.FilesContent.Tables[fileName].Rows.Add(lines[1]);
                                AppData.CurrentProject.FilesContentInfo.Tables[fileName].Rows.Add(folderName + Path.DirectorySeparatorChar + fileName);
                            }
                        }
                    }
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (FunctionsDigit.IsEqualsAnyNumberFromArray(lineNum, lineNumbers))
                    {
                        if (!string.IsNullOrEmpty(line) && !line.IsDigitsOnly() && line != "\\n")
                        {
                            ...
                            {
                                AppData.CurrentProject.FilesContent.Tables[tableName].Rows.Add(line);
                                AppData.CurrentProject.FilesContentInfo.Tables[tableName].Rows.Add(Info);
                            }
                        }
                    }
                    if (IsWrite)
                    ...
                    if (lineNum > 2 && line.Length > 0 && line != "top" && line != "bottom")
                    {
                        ...
                        for (int i = 0; i < strings.Length; i++)
                        {
                            if (!string.IsNullOrEmpty(strings[i]) && !strings[i].IsDigitsOnly())
                            {
                                ...
                                {
                                    AppData.CurrentProject.FilesContent.Tables[fileName].Rows.Add(strings[i]);
                                    AppData.CurrentProject.FilesContentInfo.Tables[fileName].Rows.Add(fileName);
                                }
                            }
                        }
                    }
