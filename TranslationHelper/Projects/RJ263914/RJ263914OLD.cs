    ...
    class RJ263914OLD
    {
        ...
        private void RubyRPGGameFIlesFromTheDir(string targetDirPath, string extension, bool IsWrite = false)
        {
            ...
            foreach (var filePath in Directory.EnumerateFiles(targetDirPath, "*." + extension, SearchOption.AllDirectories))
            {
                ...
                bool tableNotExists = ProjectData.CurrentProject.FilesContent.Tables[tableName] == null;
                if (tableNotExists)
                {
                    if (IsWrite)
                    {
                    ...
                    {
                        ProjectData.CurrentProject.FilesContent.Tables.Add(tableName);
                        ProjectData.CurrentProject.FilesContent.Tables[tableName].Columns.Add(THSettings.OriginalColumnName());

                        ProjectData.CurrentProject.FilesContent.Tables[tableName].Columns.Add(THSettings.TranslationColumnName());
                        ProjectData.Main.THFilesList.Invoke((Action)(() => ProjectData.Main.THFilesList.AddItem(tableName)));

                        ProjectData.CurrentProject.FilesContentInfo.Tables.Add(tableName);
                        ProjectData.CurrentProject.FilesContentInfo.Tables[tableName].Columns.Add(THSettings.OriginalColumnName());
                    }
                }
                ...
                else if (folderName == "pants")
                {
                    ...
                    if (IsWrite)
                    {
                        var row = ProjectData.CurrentProject.FilesContent.Tables[tableName].Rows[rowIndex];
                    ...
                    {
                        ProjectData.CurrentProject.FilesContent.Tables[tableName].Rows.Add(lineValue);
                        ProjectData.CurrentProject.FilesContentInfo.Tables[tableName].Rows.Add(fileName);
                    }
                    processingFolderName = folderName;
                    rowIndex++;
                }
                ...
                else if (folderName == "type")
                {
                    ...
                    if (lines.Length == 2 && !string.IsNullOrEmpty(lines[1]) && !lines[1].IsDigitsOnly())
                    {
                        ...
                        {
                            ProjectData.CurrentProject.FilesContent.Tables[tableName].Rows.Add(lines[1], lines[0]);
                            ProjectData.CurrentProject.FilesContentInfo.Tables[tableName].Rows.Add(fileName);
                        }
                        processingFolderName = folderName;
                        rowIndex++;
                    ...
                    if (line.Length > 0)
                    {
                        ...
                        if (lineArray.Length > 4)
                        {
                            ...
                            if (!string.IsNullOrEmpty(lineArray[4]) && !lineArray[4].IsDigitsOnly())
                            {
                                if (IsWrite)
                                {
                                    var row = ProjectData.CurrentProject.FilesContent.Tables[fileName].Rows[rowIndex];
                                ...
                                {
                                    ProjectData.CurrentProject.FilesContent.Tables[fileName].Rows.Add(lineArray[4]);
                                    ProjectData.CurrentProject.FilesContentInfo.Tables[fileName].Rows.Add(folderName + Path.DirectorySeparatorChar + fileName);
                                }
                            }
                        }
                    }
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (line.Length > 0 && line.StartsWith("message"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(lines[1]) && !lines[1].IsDigitsOnly())
                        {
                            ...
                            {
                                ProjectData.CurrentProject.FilesContent.Tables[fileName].Rows.Add(lines[1]);
                                ProjectData.CurrentProject.FilesContentInfo.Tables[fileName].Rows.Add(folderName + Path.DirectorySeparatorChar + fileName);
                            }
                        }
                    }
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (FunctionsDigit.IsEqualsAnyNumberFromArray(lineNum, lineNumbers))
                    {
                        if (!string.IsNullOrEmpty(line) && !line.IsDigitsOnly() && line != "\\n")
                        {
                            ...
                            {
                                ProjectData.CurrentProject.FilesContent.Tables[tableName].Rows.Add(line);
                                ProjectData.CurrentProject.FilesContentInfo.Tables[tableName].Rows.Add(Info);
                            }
                        }
                    }
                    if (IsWrite)
                    ...
                    if (lineNum > 2 && line.Length > 0 && line != "top" && line != "bottom")
                    {
                        ...
                        for (int i = 0; i < strings.Length; i++)
                        {
                            if (!string.IsNullOrEmpty(strings[i]) && !strings[i].IsDigitsOnly())
                            {
                                ...
                                {
                                    ProjectData.CurrentProject.FilesContent.Tables[fileName].Rows.Add(strings[i]);
                                    ProjectData.CurrentProject.FilesContentInfo.Tables[fileName].Rows.Add(fileName);
                                }
                            }
                        }
                    }
