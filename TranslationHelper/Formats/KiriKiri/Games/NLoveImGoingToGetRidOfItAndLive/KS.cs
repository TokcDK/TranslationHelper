...
namespace TranslationHelper.Formats.KiriKiri.Games.NLoveImGoingToGetRidOfItAndLive
{
    class KS : KiriKiriFormatBase
    {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (statusInfoParse)
                    {
                        if (line == "[s]")
                        {
                            ...
                            AddRowData(tablename, CheckAndRemoveRubyText(Message.ToString()).Trim(), "statusInfo:" + Environment.NewLine + MessageInfo.ToString());
        ...
        private string GetExtraSymbol(string line)
        {
        ...
        private string CheckAndRemoveRubyText(string inputString)
        {
            ...
            if (inputString.Contains("[font") || inputString.Contains("font]"))
            {
                //[font size=36]
        ...
        private string ExtraLineCorrection(string data)
        {
            if (data.Contains("[（]"))
            {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        else
                        {
                            if (line.TrimStart().StartsWith("*event_start"))
                            {
                                ...
                                if (RowData.Length < 2 || !ValidString(RowData[1]))
                                {
                                    continue;
                                }
        ...
        private void ClearData(StringBuilder Message, StringBuilder MessageInfo, ref int MessageLineNum)
        {
            Message.Clear();
            MessageInfo.Clear();
            MessageLineNum = 0;
        ...
        private string AddBracketsIfNeed(string translation, string original)
        {
