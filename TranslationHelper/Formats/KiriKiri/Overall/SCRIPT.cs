    ...
    class SCRIPT : FormatBase
    {
        ...
        private bool KiriKiriScriptScenarioOpen()
        {
            try
            {
                ...
                string tmpfile = File.ReadAllText(thDataWork.FilePath);
                if (tmpfile.Contains("[p_]") || tmpfile.Contains("[lr_]"))
                {
                    TeachingFeelingCS = true;
                }
                tmpfile = null;
                using (StreamReader file = new StreamReader(thDataWork.FilePath, TeachingFeelingCS ? Encoding.UTF8 : Encoding.GetEncoding(932)))
                {
                    string line;
                        ...
                        else
                        {
                            if (TeachingFeelingCS)//Teaching Feeling cs
                            {
                                if (line.Contains("[p_]") || line.Contains("[lr_]"))
                                {
                                    ...
                                    {
                                        _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(line);
                                        _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add("Teeching feeling cs");
                                    }
                                }
                            }
                            else
                            {
                                if (line == "@nanasi")//Life with daughter
                                {
                                    ...
                                    _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(line);
                                    _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add(string.Empty);
                                }
                                else if (line.EndsWith("[k]")) // text ;Magic Swordsman Rene
                                {
                                    ...
                                    {
                                        _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(line);
                                        _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add("[k] = end of line");
                                    }
                                }
                                ...
                                else if (line.EndsWith("[r]")) //text, first line ;Magic Swordsman Rene
                                {
                                    ...
                                    {
                                        _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(line);
                                        _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add("[r] = carriage return");
                                    }
                                }
                                else if (line.StartsWith("o.") || Regex.IsMatch(line, KiriKiriVariableSearchRegexPattern)) //variable, which is using even for displaing and should be translated in all files ;Magic Swordsman Rene
                                {
                                    ...
                                    {
                                        _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(matches[m].Value.Remove(0, startswith ? 2 : 3));
                                        _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add(T._("Variable>Must be Identical in all files>Only A-Za-z0-9" + Environment.NewLine + "line: " + line));
                                        if (startswith)
                                        {
                                ...
                                else if (line.StartsWith("@notice text="))// ; Magic Swordsman Rene
                                {
                                    ...
                                    {
                                        _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(line);
                                        _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add("@notice text=");
                                    }
                                }
                                ...
                                else if (Regex.IsMatch(line, "\\\"(.*?)\\\""))// ; Magic Swordsman Rene
                                {
                                    ...
                                    foreach (var m in matches)
                                    {
                                        ...
                                        {
                                            _ = thDataWork.THFilesElementsDataset.Tables[fileName].Rows.Add(subline.Remove(subline.Length - 1, 1).Remove(0, 1));
                                            _ = thDataWork.THFilesElementsDatasetInfo.Tables[fileName].Rows.Add(line);
                                        }
                                    }
                                }
                            }
