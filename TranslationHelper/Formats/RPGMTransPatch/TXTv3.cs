...
namespace TranslationHelper.Formats.RPGMTrans
{
    ...
    {
        public TXTv3(THDataWork thData, StringBuilder sBuffer, int tableIndex) : base(thData, sBuffer, tableIndex)
        {
        }
        ...
        internal override bool Save()
        {
            try
            {
                ...
                for (int y = 0; y < thDataWork.THFilesElementsDataset.Tables[TableIndex].Rows.Count; y++)
                {
                    ...
                    buffer.AppendLine(base.thDataWork.THFilesElementsDataset.Tables[TableIndex].Rows[y][originalcolumnindex] + string.Empty);// + Environment.NewLine);
                    ...
                    string[] CONTEXT = (thDataWork.THFilesElementsDataset.Tables[TableIndex].Rows[y][contextcolumnindex] + string.Empty).Split(new string[1] { Environment.NewLine }, StringSplitOptions.None/*'\n'*/);
